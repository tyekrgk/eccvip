...
class IndexviewController extends BaseController{
	...
	public function edit(){
		...
		if($id){
			$new=M("indexview")->field("title,content,id")->where("id=".$id)->find();
		}
		$this->new=$new;
		$this->display();
	}
	...
	public function save(){
		...
		if(!$data['title']){
			...
			if($id){
				if(M("indexview")->where("id=".$id)->save($data)){
					$edata="修改成功";
					$status=1;
				}else{
					$edata="修改失败";
	...
	public function delete(){
		$id=I("get.id",0,"intval");
		$type=0;
		if(M("indexview")->where("id=".$id)->delete()){
			$edata="操作成功";
			$type=1;
		}else{
			$edata="操作失败";
