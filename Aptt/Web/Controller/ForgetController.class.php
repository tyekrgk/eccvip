...
class ForgetController extends BaseController{
	...
	public function cheusername(){
		...
		if($data){
			 $this->show(alertstr($data,1,U(C("FILE_IN").'/Forget/index')),"utf-8");
			 exit;
		}
	...
	public function next(){
		if (IS_POST){
			...
			if($data==1){
			    ...
			    $this->show(alertstr("密码重置成功",0,U(C("FILE_IN").'/Forget/showinfo',array("username"=>$username,"up"=>$str))),"utf-8");
			}else{
				$this->show(alertstr($data,2),"utf-8");
			}
		}else{
			$this->show(alertstr("请勿非法操作",0,U(C("FILE_IN").'/Forget/index')),"utf-8");
		}
	}
	...
	public function resetpass(){
		if (IS_POST){
			...
			if($data==1){
			    ...
			    M("member")->data(array("upd1"=>md5($str),"upd2"=>md5($str),"code"=>"","codeover"=>""))->where(array("username"=>$username))->save();
			    $this->username=$username;
				$this->up=$str;
				$this->display();
			}else{
